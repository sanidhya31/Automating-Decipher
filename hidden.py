...
def ques_hid(wholeques):
    ...
    for eachline in wholeques:
        ...
        if first_line:  # Radio and title tag
            radio_str = '<exec>\n\n</exec>\n<radio label="h{}" optional="1" where="execute,survey,report">\n'.format(eachline[0])
            radio_str += '<title>Hidden Variable: {}</title>\n'.format(' '.join(eachline[1:]))
            first_line = False
        else:  # all about rows
            counter_for_row_label += 1
            attrs = ('randomize="0"' if val_OE == counter_for_row_label else '') + (
                'randomize="0"' if val_exc == counter_for_row_label else '')
